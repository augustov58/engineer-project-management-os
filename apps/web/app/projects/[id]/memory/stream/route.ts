...
/**
 * A job's memory runs and proposals as they move, proxied so the browser
 * never calls the API.
 *
 * The reports stream written a third time (issues #12, #13), with the same
...
export async function GET(
  ...
  const upstream = await fetch(
    apiPath(`/projects/${encodeURIComponent(id)}/memory/stream`),
    {
      cache: 'no-store',
