...
#15 — and **step 6, curated project memory, is complete** — issue #18, with the three correctness gaps found reviewing it closed as issue #42. Issue #16, the
...
- `project_memory_versions.seq` is the **tie-break and nothing else** — never an identifier, never on the wire, not a count (issue #42). "The current memory is the latest" ordered on `created_at` then `id`, and `id` is a random v4 uuid while `created_at` is `TIMESTAMP(3)`, so two versions written in the same millisecond ordered by coin toss and the older could read as current. The order lives in one place both the memory read and the proposal's base lookup share (`currentContent`); do not spell an `orderBy` for this table at a call site.
